...
def test_import():
    """Test that we can import zpybci."""
    import zpybci as zbci
    assert hasattr(zbci, '__version__')
    assert hasattr(zbci, 'IirFilter')
...
def test_butterworth_highpass_creation():
    ...
    hpf = zbci.IirFilter.butterworth_highpass(1000.0, 1.0)
...
def test_invalid_cutoff():
    ...
    with pytest.raises(ValueError):
        zbci.IirFilter.butterworth_lowpass(1000.0, 600.0)

    # Negative cutoff
    with pytest.raises(ValueError):
        zbci.IirFilter.butterworth_lowpass(1000.0, -10.0)

    # Bandpass with invalid range
    with pytest.raises(ValueError):
        zbci.IirFilter.butterworth_bandpass(1000.0, 50.0, 40.0)
...
def test_lowpass_attenuates_high_freq():
    ...
    sample_rate = 1000.0
    lpf = zbci.IirFilter.butterworth_lowpass(sample_rate, 30.0)
