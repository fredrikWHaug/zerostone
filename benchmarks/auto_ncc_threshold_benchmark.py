"""Benchmark: auto_ncc_threshold — NCC template-scan recovery for large probes.

Baseline (auto_refine_iterations=3 default): Easy 82.7%, Medium 73.4%, Hard 64.2%, Avg 73.4%

auto_ncc_threshold=true lowers the NCC threshold by 7% for C>=8 (0.70 → 0.651).
On large probes, spatial deduplication and coincidence detection provide an
additional false-positive gate, so a looser shape-match bar can recover
sub-threshold spikes that closely but not perfectly match a template.
"""
...
def run(difficulty, label, detail=False, **kwargs):
    ...
    defaults = dict(
        ...
        ncc_threshold=0.70, auto_ncc_threshold=True,
    )
...
def main():
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        # Default: auto_ncc_threshold=True (0.70 → 0.651 for C>=8)
        acc_on, pu_on = run(
            difficulty, "auto_ncc_threshold=True   (0.70 → 0.651 for C>=8)", detail=detail,
        )

        # Off: keep 0.70 hardcoded behavior
        acc_off, _ = run(
            difficulty, "auto_ncc_threshold=False  (0.70 fixed)",
            auto_ncc_threshold=False,
        )

        # Tight: 0.75 fixed (more conservative)
        acc_tight, _ = run(
            difficulty, "ncc_threshold=0.75 auto_ncc=False (conservative)",
            ncc_threshold=0.75, auto_ncc_threshold=False,
        )

        # Loose: 0.60 fixed (aggressive recovery, more FP risk)
        acc_loose, _ = run(
            difficulty, "ncc_threshold=0.60 auto_ncc=False (aggressive, FP risk)",
            ncc_threshold=0.60, auto_ncc_threshold=False,
        )
    ...
    print(f"{'='*88}")
    configs = [
        ("ncc_threshold=0.75 fixed (conservative)",           "tight_075"),
        ("ncc_threshold=0.70 fixed (old behavior)",           "auto_off"),
        ("auto_ncc_threshold=True  (0.70→0.651 for C>=8)",    "auto_on"),
        ("ncc_threshold=0.60 fixed (aggressive FP risk)",     "loose_060"),
    ]
    ...
    out_path = f"benchmarks/results/auto_ncc_threshold_{ts}.json"
