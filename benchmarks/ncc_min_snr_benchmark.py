"""Benchmark: ncc_min_snr — per-template SNR gate for NCC threshold relaxation.

Baseline: Easy 82.7%, Medium 73.4%, Hard 64.2%, Avg 73.4%

Problem: global NCC lowering helped unit 12 (+4.1%) but collapsed unit 1 (0%).
Solution: per-template SNR gate. High-SNR templates (tmpl_rms >= ncc_min_snr) use
0.70 × 0.93 = 0.651. Low-SNR templates (like unit 1) stay at 0.70.

tmpl_rms = t_norm / sqrt(W). For whitened data with W=48:
  SNR ~5σ unit: tmpl_rms ≈ 2.5  (below 4.0 → keeps 0.70)
  SNR ~8σ unit: tmpl_rms ≈ 4.0  (at threshold)
  SNR ~10σ unit: tmpl_rms ≈ 5.0 (above threshold → gets 0.651)
"""
...
def run(difficulty, label, detail=False, **kwargs):
    ...
    defaults = dict(
        ...
        template_subtract_passes=2, ncc_min_snr=4.0,
    )
...
def main():
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        # Baseline: ncc_min_snr=inf (no relaxation, equivalent to old behavior)
        acc_off, _ = run(difficulty, "ncc_min_snr=inf  (no relaxation, old behavior)",
                         ncc_min_snr=float("inf"))
        # Default: ncc_min_snr=4.0
        acc_4, pu4 = run(difficulty, "ncc_min_snr=4.0  (default: ~SNR8+ gets 0.651)",
                         ncc_min_snr=4.0, detail=detail)
        # More conservative: ncc_min_snr=5.0
        acc_5, _ = run(difficulty, "ncc_min_snr=5.0  (~SNR10+ gets 0.651)",
                       ncc_min_snr=5.0)
        # More aggressive: ncc_min_snr=3.0
        acc_3, _ = run(difficulty, "ncc_min_snr=3.0  (~SNR6+ gets 0.651)",
                       ncc_min_snr=3.0)
        # Fully relaxed: ncc_min_snr=0 (all templates get 0.651)
        acc_0, _ = run(difficulty, "ncc_min_snr=0.0  (all templates get 0.651, old problem)",
                       ncc_min_snr=0.0)
        ...
        results[difficulty] = {
            ...
            "snr3": acc_3,
            "snr0": acc_0,
            "per_unit_snr4": {str(k): float(v) for k, v in pu4.items()},
        }
    ...
    print(f"{'='*92}")
    for label, key in [
        ("ncc_min_snr=inf  (disabled, old behavior)",       "off"),
        ("ncc_min_snr=5.0  (only strongest get 0.651)",     "snr5"),
        ("ncc_min_snr=4.0  (default)",                      "snr4"),
        ("ncc_min_snr=3.0  (moderate)",                     "snr3"),
        ("ncc_min_snr=0.0  (all templates — old problem)",  "snr0"),
    ]:
        e = results["easy"][key]
    ...
    out_path = f"benchmarks/results/ncc_min_snr_{ts}.json"
