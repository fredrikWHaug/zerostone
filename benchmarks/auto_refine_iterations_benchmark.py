"""Benchmark: auto_refine_iterations — minimum k-means refinement passes for large probes.

Baseline (refinement_iterations=1): 73.3% avg (Easy 82.7%, Medium 73.4%, Hard 63.8%)

The refinement loop already has a convergence break (labels unchanged → stop early).
auto_refine_iterations=N (default: 3) sets a per-probe minimum: when auto_refine && C>=8,
effective_iters = max(refinement_iterations, auto_refine_iterations).

Key question: does allowing up to 3 convergent passes improve hard unit recovery
without regressing easy (gated at C<8) or medium (ISI guard prevents degradation)?
"""
...
BASELINE = {"easy": 0.8270, "medium": 0.7340, "hard": 0.6380}
...
def main():
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        # Baseline: current defaults (auto_refine_iterations=3 is new default)
        acc_base, pu_base = run(
            difficulty, "current defaults (auto_refine_iterations=3)", detail=detail,
        )

        # Explicit 1-iteration reference (old behavior)
        acc_iter1, _ = run(
            difficulty, "auto_refine_iterations=1  (old single-pass behavior)",
            auto_refine_iterations=1,
        )

        # 2 iterations
        acc_iter2, _ = run(
            difficulty, "auto_refine_iterations=2",
            auto_refine_iterations=2,
        )

        # 5 iterations (convergence ceiling)
        acc_iter5, _ = run(
            difficulty, "auto_refine_iterations=5  (convergence ceiling)",
            auto_refine_iterations=5,
        )

        results[difficulty] = {
            "iter3_default": acc_base,
            "iter1": acc_iter1,
            "iter2": acc_iter2,
            "iter5": acc_iter5,
            "per_unit_iter3": {str(k): float(v) for k, v in pu_base.items()},
        }
    ...
    print(f"{'='*88}")
    configs = [
        ("auto_refine_iterations=1  (old single-pass)",  "iter1"),
        ("auto_refine_iterations=2",                      "iter2"),
        ("auto_refine_iterations=3  (new default)",       "iter3_default"),
        ("auto_refine_iterations=5  (convergence ceil)",  "iter5"),
    ]
    ...
    out_path = f"benchmarks/results/auto_refine_iterations_{ts}.json"
