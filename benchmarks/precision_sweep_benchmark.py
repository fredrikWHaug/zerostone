"""Benchmark: targeting precision-limited hard units 1, 8, 12.

Unit 1:  R=0.927, P=0.544 — cluster absorbs ~40% false positives
Unit 8:  R=0.938, P=0.655 — ~30% false positives
Unit 12: R=0.996, P=0.718 — ~28% false positives

These clusters have correct spikes but too many wrong ones mixed in.
Try: isi_split_threshold, cluster_threshold, matched_filter_threshold.
"""
...
def run(difficulty, label, detail=False, **kwargs):
    ...
    n_gt = len(gt_trains)
    if n_spikes == 0:
        print(f"  {label:<68} acc=0.000  {elapsed:.1f}s")
        return 0.0
    ...
    print(f"  {label:<68} acc={acc:.3f} ({delta:+.3f})  wd={wd}/{n_gt}  {n_spikes}spk/{n_cl}cl  {elapsed:.1f}s")
    if detail:
...
def main():
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        # ISI split: more aggressive splitting of contaminated clusters
        for ist in [0.08, 0.05, 0.03]:
            r[f"ist_{ist}"] = run(difficulty, f"isi_split_threshold={ist}",
                                  isi_split_threshold=ist, detail=detail)

        # Lower cluster_threshold: create more initial clusters
        for ct in [6.0, 5.0]:
            r[f"ct_{ct}"] = run(difficulty, f"cluster_threshold={ct}",
                                cluster_threshold=ct, detail=detail)

        # Higher matched_filter_threshold: fewer false-positive MF detections
        for mft in [4.0, 4.5, 5.0]:
            r[f"mft_{mft}"] = run(difficulty, f"matched_filter_threshold={mft}",
                                   matched_filter_threshold=mft, detail=detail)

        # Tighter merge_dprime (reduce over-merging that mixes units)
        r["mdp_2.5"] = run(difficulty, "merge_dprime_threshold=2.5",
                            merge_dprime_threshold=2.5, detail=detail)

        # refine_isi_guard=True (prevent refinement from contaminating)
        r["isi_guard"] = run(difficulty, "refine_isi_guard=True",
                              refine_isi_guard=True, detail=detail)
    ...
    for label, key in [
        ("baseline", "baseline"),
        ("isi_split=0.08", "ist_0.08"),
        ("isi_split=0.05", "ist_0.05"),
        ("isi_split=0.03", "ist_0.03"),
        ("cluster_threshold=6.0", "ct_6.0"),
        ("cluster_threshold=5.0", "ct_5.0"),
        ("matched_filter_threshold=4.0", "mft_4.0"),
        ("matched_filter_threshold=4.5", "mft_4.5"),
        ("matched_filter_threshold=5.0", "mft_5.0"),
        ("merge_dprime=2.5", "mdp_2.5"),
        ("refine_isi_guard=True", "isi_guard"),
    ]:
